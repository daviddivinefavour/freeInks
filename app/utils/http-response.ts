...
export const HTTP_204 = (customMessage?: string): IHttpResponse<undefined> => ({
  statusCode: 204,
  title: 'No Content',
  message: customMessage || 'Request successfully processed, but no content to return',
});

export const HTTP_400 = (customMessage?: string): IHttpResponse<undefined> => ({
  statusCode: 400,
  title: 'Bad Request',
  message: customMessage || 'The server could not understand the request',
});

export const HTTP_401 = (customMessage?: string): IHttpResponse<undefined> => ({
  statusCode: 401,
  title: 'Unauthorized',
  message: customMessage || 'Authentication is required and has failed or not yet been provided',
});

export const HTTP_403 = (customMessage?: string): IHttpResponse<undefined> => ({
  statusCode: 403,
  title: 'Forbidden',
  message: customMessage || 'You are not authorized to perform this action',
});

export const HTTP_404 = (customMessage?: string): IHttpResponse<undefined> => ({
  statusCode: 404,
  title: 'Not Found',
  message: customMessage || 'The requested resource could not be found',
});

export const HTTP_422 = (customMessage?: string): IHttpResponse<undefined> => ({
  statusCode: 422,
  title: 'Unprocessable Entity',
  message: customMessage || 'An error occurred',
});

export const HTTP_500 = (customMessage?: string): IHttpResponse<undefined> => ({
  statusCode: 500,
